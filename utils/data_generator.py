...
class AudioSetDatasetCsv(object):
    ...
    def __getitem__(self, df_row: dict):
        ...
        data_dict = {
            ...
            "waveform": waveform,
            "target": df_row["label"],
        }  # TODO: figure out what target does
    ...
    def __getitem__(self, meta):
        ...
        Returns:
          data_dict: {
            'audio_name': str,
            'waveform': (clip_samples,),
            'target': (classes_num,)} # TODO: What is target?
        """
        ...
        for i in range(max_len + 1 - self.batch_size):
            for label in self.label_dicts:
                for item in (
                    ...
                    .slice(i % self.label_dicts[label].shape[0], self.batch_size)
                    .to_dicts()
                ):
...
class TrainSampler(Base):
    def __init__(
        self, indexes_hdf5_path, batch_size, black_list_csv=None, random_seed=1234
    ):
    ...
    def state_dict(self):
        state = {"indexes": self.indexes, "pointer": self.pointer}
        return state
...
class BalancedTrainSampler(Base):
    def __init__(
        ...
        Args:
          ...
          random_seed: int
        """
        super(BalancedTrainSampler, self).__init__(
            indexes_hdf5_path, batch_size, black_list_csv, random_seed
        )

        self.samples_num_per_class = np.sum(self.targets, axis=0)
        logging.info(
            "samples_num_per_class: {}".format(
    ...
    def __iter__(self):
        ...
        while True:
            ...
            while i < batch_size:
                ...
                if (
                    self.pointers_of_classes[class_id]
                    >= self.samples_num_per_class[class_id]
                ):
...
class AlternateTrainSampler(Base):
    def __init__(
        ...
        Args:
          ...
          random_seed: int
        """
        self.sampler1 = TrainSampler(
            indexes_hdf5_path, batch_size, black_list_csv, random_seed
        )

        self.sampler2 = BalancedTrainSampler(
            indexes_hdf5_path, batch_size, black_list_csv, random_seed
        )
        ...
        while True:
            ...
            if self.count % 2 == 0:
                ...
                while i < batch_size:
                    ...
                    # If audio in black list then continue
                    if (
                        self.sampler1.audio_names[index]
                        in self.sampler1.black_list_names
                    ):
                        continue
                    else:
                        batch_meta.append(
                ...
                while i < batch_size:
                    if len(self.sampler2.queue) == 0:
                        self.sampler2.queue = self.sampler2.expand_queue(
                            self.sampler2.queue
                        )
                    ...
                    # When finish one epoch of a sound class, then shuffle its indexes and reset pointer
                    if (
                        self.sampler2.pointers_of_classes[class_id]
                        >= self.sampler2.samples_num_per_class[class_id]
                    ):
                        self.sampler2.pointers_of_classes[class_id] = 0
                        self.sampler2.random_state.shuffle(
                            self.sampler2.indexes_per_class[class_id]
                        )

                    # If audio in black list then continue
                    if (
                        self.sampler2.audio_names[index]
                        in self.sampler2.black_list_names
                    ):
                        continue
                    else:
                        batch_meta.append(
